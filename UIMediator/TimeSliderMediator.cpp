...
{
  this->m_ui->timeSlider->setStartTime(tv);
}

void
UIMediator::setSourceTimeEnd(struct timeval const &tv)
{
  this->m_ui->timeSlider->setEndTime(tv);
}
...
{
  m_lastTimeStamp = tv;
  this->m_ui->timeSlider->setTimeStamp(tv);
}
...
{
  emit seek(this->m_ui->timeSlider->getTimeStamp());
}
